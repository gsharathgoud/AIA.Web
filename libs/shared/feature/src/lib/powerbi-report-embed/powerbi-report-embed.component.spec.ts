...
describe('PowerBIReportEmbedComponent', () => {
  ...
  describe('Interaction with Power BI service', () => {
    ...
    beforeEach(() => {
      mockPowerBIService = createSpyObj('mockService', ['load', 'embed', 'bootstrap']);
    });
    ...
    it('first bootstraps, then embeds when accessToken is available', () => {
      ...
      // Reset for next Act
      mockPowerBIService.embed.mockClear();
      mockPowerBIService.bootstrap.mockClear();
  ...
  describe('Tests for setting event handlers', () => {
    ...
    it('clears previous event handlers and sets new event handlers', () => {
      ...
      jest.spyOn(testReport, 'on');
      jest.spyOn(testReport, 'off');
    ...
    it('does not set the same eventHandler map again', () => {
      ...
      const spyForOn = jest.spyOn(testReport, 'on');
      const spyForOff = jest.spyOn(testReport, 'off');
      component.eventHandlers = eventHandlers;
      component.ngOnChanges({
        eventHandlers: new SimpleChange(undefined, component.eventHandlers, true),
      });
  ...
  describe('Test for get report', () => {
    it('returns the report component', () => {
      ...
      // Assert
      expect(response.config).toEqual(expect.objectContaining(expectedResponse.config));
    });
  });
  ...
  describe('Tests for report features', () => {
    ...
    describe('Test of report level filters', () => {
      it('returns a list of applied filters', async () => {
        ...
        jest.spyOn(testReport, 'getFilters').mockReturnValue(resolvedPromise);

        // Act
        const filters = await testReport.getFilters();

        // Assert
        expect(filters).toEqual(expect.objectContaining(fakeFilters));
      });
      ...
      it('sets filters', async () => {
        ...
        jest.spyOn(testReport, 'setFilters').mockReturnValue(resolvedPromise);
      ...
      it('removes filter', async () => {
        ...
        const resolvedPromise = Promise.resolve(expectedResponse);
        jest.spyOn(testReport, 'removeFilters').mockReturnValue(resolvedPromise);
      ...
      it('updates filter', async () => {
        ...
        const resolvedPromise = Promise.resolve(expectedResponse);
        jest.spyOn(testReport, 'updateFilters').mockReturnValue(resolvedPromise);

        // Act
        const response = await testReport.updateFilters(models.FiltersOperations.Replace, [
          testFilter,
        ]);
    ...
    describe('Tests of page features', () => {
      it('sets given page as active page', async () => {
        ...
        const resolvedPromise = Promise.resolve(expectedResponse);
        jest.spyOn(testReport, 'setPage').mockReturnValue(resolvedPromise);
      ...
      it('returns all the pages of a report', async () => {
        ...
        jest.spyOn(testReport, 'getPages').mockReturnValue(resolvedPromise);
    ...
    describe('Tests of update settings', () => {
      it('updates report settings', async () => {
        ...
        const resolvedPromise = Promise.resolve(expectedResponse);
        jest.spyOn(testReport, 'updateSettings').mockReturnValue(resolvedPromise);
