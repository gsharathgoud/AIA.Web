...
describe('PowerBIVisualEmbedComponent', () => {
  ...
  describe('Interaction with Power BI service', () => {
    ...
    beforeEach(() => {
      mockPowerBIService = createSpyObj('mockService', ['embed', 'bootstrap']);
    });
  ...
  describe('Tests for setting event handlers', () => {
    ...
    it('clears previous event handlers and sets new event handlers', () => {
      ...
      jest.spyOn(testVisual, 'on');
      jest.spyOn(testVisual, 'off');
    ...
    it('does not set the same eventHandler map again', () => {
      ...
      const spyForOn = jest.spyOn(testVisual, 'on');
      const spyForOff = jest.spyOn(testVisual, 'off');
      component.eventHandlers = eventHandlers;
      component.ngOnChanges({
        eventHandlers: new SimpleChange(undefined, component.eventHandlers, true),
      });
  ...
  describe('Tests for visual features', () => {
    ...
    it('returns a list of applied filters', async () => {
      ...
      jest.spyOn(testVisual, 'getFilters').mockReturnValue(resolvedPromise);
    ...
    it('sets filters', async () => {
      ...
      jest.spyOn(testVisual, 'setFilters').mockReturnValue(resolvedPromise);
    ...
    it('removes filter', async () => {
      ...
      const resolvedPromise = Promise.resolve(expectedResponse);
      jest.spyOn(testVisual, 'removeFilters').mockReturnValue(resolvedPromise);
    ...
    it('updates filter', async () => {
      ...
      const resolvedPromise = Promise.resolve(expectedResponse);
      jest.spyOn(testVisual, 'updateFilters').mockReturnValue(resolvedPromise);

      // Act
      const response = await testVisual.updateFilters(models.FiltersOperations.Replace, [
        testFilter,
      ]);
